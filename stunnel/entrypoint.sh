...
if [[ ! -e /usr/pki-example-2 ]]; then
    #Just scripting http://pki-tutorial.readthedocs.io/en/latest/advanced/index.html
    cd /usr && git clone https://bitbucket.org/stefanholek/pki-example-2 && cd pki-example-2 
    ...
    # Create ROOT CA database, request and sign it
...
[monerod] 
accept = 30000 
connect = 192.168.1.40:18081
sslVersion = TLSv1.2
verify = 2
CAfile = /usr/pki-example-2/ca/tls-ca-chain.pem 
CRLfile = /usr/pki-example-2/crl/tls-ca-chain.crl 
cert = /usr/pki-example-2/certs/green-chain.pem 
key = /usr/pki-example-2/certs/green.no.key" > /usr/stunnel.conf
